...
if command -v docker &> /dev/null; then
    ...
    sudo curl -fsSL https://download.docker.com/linux/debian -o /etc/apt/keyrings/docker.asc
...
if ! getent group docker | grep -q "\b$(whoami)\b"; then
    ...
    echo "[IMPORTANT] You must RESTART your machine for this change to take effect."
fi
...
if [ -f "$CONFIG_DIR/config.json" ]; then
    ...
    echo ">>> Copying default launcher configuration..."
